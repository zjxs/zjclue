Vue.component('sy-treeselect', {
	...
	template: ` ,
	<div>
		<Select v-model="inputText" :value="value" :placeholder="placeholder" style="width:100%">,
		<Option style ="display:none" v-for="item in list" :value="item.value" :key="item.value">{{ item.label }}</Option>
			<Tree @on-select-change="ok()" @on-check-change="ok()" :ref="tablename" :data="treedata" :load-data="loadData" :multiple="multiple" :show-checkbox="showCheckbox" :check-strictly="checkDirectly" :check-directly="checkDirectly" :check-strictly="checkStrictly"></Tree>
	...
	methods: {
		...
		ok() {
			...
			this.list[0].value=this.inputText;
			this.list[0].label=this.inputText;
...
window.utils_select = utils_select;
